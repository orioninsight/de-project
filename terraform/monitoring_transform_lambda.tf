resource "aws_cloudwatch_log_metric_filter" "transform_errors_filter" {
  name           = "error_filter"
  pattern        = "ERROR"
  log_group_name = aws_cloudwatch_log_group.transformation_lambda_log.name
...
resource "aws_cloudwatch_metric_alarm" "transformation_lambda_errors_alarm" {
  alarm_name = "transformation-lambda-error-alarm"
