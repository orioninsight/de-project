...
def extract_db_handler(event, context):
    """ AWS Lambda to extract tables from Totesys DB
        ...
        Relies on environmental variable 'OI_TOTESYS_SECRET_STRING' or AWS secret called 'totesys_db' to be
    ...
    """
    try:
        if event == None or 'extract_table' not in event:
            raise Exception("Event payload requires list in 'extract_table'")
        db_secret_string = os.environ['OI_TOTESYS_SECRET_STRING'] if 'OI_TOTESYS_SECRET_STRING' in os.environ else retrieve_entry(
            'totesys_db')
        db_secret_json = json.loads(db_secret_string)
        extractor = Extractor(**db_secret_json)
        saver = Saver()
        storer_secret_string = os.environ['OI_STORER_SECRET_STRING'] if 'OI_STORER_SECRET_STRING' in os.environ else retrieve_entry(
            'storer_info')
            ...
            for table in tables_to_extract:
                if table in ('address', 'design'):
                    ...
                    if not storer.store_file(file_name, table):
                        raise Exception(f"Could not store data file")
