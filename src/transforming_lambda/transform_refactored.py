...
def transform_lambda_handler(event, context):
    ...
    data_frames = transformer.read_csv(files)
...
class Transformer:
    def __init__(self):
        ...
        self.s3_bucket_json = json.loads(os.environ['OI_STORER_SECRET_STRING'])
        self.ingestion_bucket = self.s3_client.Bucket(
            self.s3_bucket_json['s3_bucket_name'])
    ...
    def list_csv_files(self):
        ...
        ingestion_csv_files = self.ingestion_bucket.objects.all()
